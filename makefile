...
ASSRCS			=	crt0.s \
					...
					App.s \
					Ring.s \
					Game.s Player.s Enemy.s Shot.s Back.s Firework.s \
					rtable.s \
					pattern.s
...
resource:
	...
	@$(TOOLDIR)/rtable -r 88 -l 256 -d 256 -s 16 -x 128 -y 96 -n rTable -o sources/rtable.s
...
tool:
	...
	@gcc -o $(TOOLDIR)/rtable $(TOOLDIR)/rtable.cpp
